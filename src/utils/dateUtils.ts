export const calculateDuration = (startDate: string, endDate?: string): string => {
  ...
  const end = endDate && endDate.trim() ? new Date(endDate + "-01") : new Date();
...
export const calculateTotalExperience = (experience: readonly { startDate: string; endDate?: string }[]): number => {
...
export const formatDateRange = (startDate: string, endDate?: string): string => {
  ...
  const formattedEnd = endDate && endDate.trim() ? formatDate(endDate) : "Present";
