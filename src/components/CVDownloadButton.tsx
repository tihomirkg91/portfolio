...
export const CVDownloadButton: React.FC<CVDownloadButtonProps> = ({ showPreview = false, className = "" }) => {
  ...
  return (
    <div className={`about-cta ${className}`}>
      ...
      >
        {currentAction === "download" || isGenerating ? "Generating CV..." : "Download Resume"}
      </button>
      ...
      {showPreview && (
        ...
        >
          {currentAction === "preview" || isGenerating ? "Loading Preview..." : "Preview CV"}
        </button>
      )}
    </div>
  );
